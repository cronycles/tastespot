...
5. If the user story lacks the technical and specific detail necessary to allow the developer to be fully autonomous when completing it, provide an improved story that is clearer, more specific, and more concise in line with product best practices described in step 4. Use technical context from project documentation in `docs/` (especially `docs/base-standards.mdc`, `docs/backend-standards.mdc`, `docs/frontend-standards.mdc`, `docs/data-model.md`, `docs/api-spec.yml`, and `docs/documentation-standards.mdc`). Return it in markdown format.
...
- Do not remove original ticket text; append enhanced content.
- Keep all technical artifacts in English.
- If backend and frontend are both impacted, provide separate implementation notes for each side.
